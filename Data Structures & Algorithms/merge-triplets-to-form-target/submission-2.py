...
3. filter out triplets with values greater than target, merge all and if valid return true else false. T: O(n) S: O(1)
"""

class Solution:
    def mergeTriplets(self, triplets: List[List[int]], target: List[int]) -> bool:
        #tracks candidate triplet
        result = [0,0,0]

        #compiles all triplets with values less than or equal to target
